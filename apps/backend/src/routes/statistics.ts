...
async function statsRoutes(req: express.Request, res: express.Response) {
    try {
        ...
        const underwriterCount = employees.filter((e) =>
            e.roles.includes("UnderWriter"),
        ).length;
        const analystCount = employees.filter((e) =>
            e.roles.includes("BusinessAnalyst"),
        ).length;
        const busOpCount = employees.filter((e) =>
            e.roles.includes("BusinessOperator"),
        ).length;
        const exOpCount = employees.filter((e) =>
            e.roles.includes("ExcelOperator"),
        ).length;
        const acCount = employees.filter((e) =>
            e.roles.includes("ActuarialAnalyst"),
        ).length;
        const adminCount = employees.filter((e) =>
            e.roles.includes("Administrator"),
        ).length;
