...
async function statsRoutes(req: express.Request, res: express.Response) {
    try {
        ...
        const underwriterCount = employees.filter((e: Employee) => e.roles.includes("UnderWriter")).length
        const analystCount = employees.filter((e: Employee) => e.roles.includes("BusinessAnalyst")).length
