...
console.log(
    `Allowing server ${process.env.FRONTEND_SERVER} communication in CORS config.`,
);
...
app.post(
    "/create-employee",
    /* requireAuth(),*/ validate(CreateEmployeeModel),
    createOldEmployeeRoute,
);
app.post(
    "/get-link-role",
    /* requireAuth(),*/ validate(LinkRoleModel),
    linkRoleRoute,
);
app.post(
    "/update-favorite",
    /* requireAuth(),*/ validate(UpdateFavoriteModel),
    updateFavoriteRoute,
);
app.post("/update-favorite-link", /* requireAuth(),*/ updateFavoriteLinksRoute);
app.post(
    "/edit-employee",
    /* requireAuth(),*/ validate(EditEmployeeModel),
    editEmployeeRoute,
);
...
app.listen(PORT, () => {
    console.log(
        `\x1b[33mServer started on\x1b[36m http://localhost:${PORT}!\x1b[0m`,
    );
