...
const intClock = async () => {
    ...
    if (removedNotifs.deletedCount > 0)
        console.log(
            `[InitClock] Deleted ${removedNotifs.deletedCount} notifications.`,
        );
    if (removedNotifs.cleanedEmployeesCount > 0)
        console.log(
            `[InitClock] Cleaned up ${removedNotifs.cleanedEmployeesCount} employees.`,
        );
    if (removedNotifs.deletedNotificationIds.length > 0)
        console.log(
            `[InitClock] Deleted notification IDs: ${removedNotifs.deletedNotificationIds.join(", ")}`,
        );
};
