...
export default function LinksTable<TData extends Links, TValue>({
  ...
  async function toggleFavorite(link: Links, nextValue: boolean) {
    try {
      ...
      setLinks((prev) =>
        prev.map((item) =>
          item.id === link.id ? { ...item, favorite: nextValue } : item,
        ),
      )
    } catch (error) {
      console.error(error)
            ...
            <div className="flex items-center gap-2">
              <InputGroup className="flex-1 max-w-2xl h-8 border-2 shadow-md hover:shadow-xl transition-all duration-100 bg-white">
                <InputGroupInput
                  placeholder="Search"
                  value={
                    (table.getColumn("link_name")?.getFilterValue() as string) ??
                    ""
                  }
                  onChange={(event) =>
                    table
              ...
              <TableBody>
                {table.getRowModel().rows.map((row) => {
                  ...
                  return (
                    <TableRow
                      key={row.id}
                      className={isLockedByOther ? "bg-[#e6e8e8]" : ""}
                    >
                      <FavoriteStar
                        doc={link}
                        onToggleOn={() => toggleFavorite(link, false)}
                        onToggleOff={() => toggleFavorite(link, true)}
                      />
