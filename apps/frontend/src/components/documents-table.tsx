            ...
            <div key={group.key} className="relative inline-block text-left">
              <div className="flex gap-x-0.5">
                <button
                  onClick={() =>
                    onActiveGroupChange(
                      activeGroup === group.key ? null : group.key,
                    )
                  }
    ...
    <div className="py-1 mb-2 flex flex-row flex-wrap gap-2">
      {filters.map((filter) => (
        ...
        >
          ...
          <button onClick={() => onRemove(filter)} className="text-black pr-2">
            <span className="ml-1">
              <HugeiconsIcon size={16} icon={X} />
            </span>
          </button>
  ...
  const selectedFilters = React.useMemo(() => {
    ...
    if (tab === "OwnedByMe") {
      return [
        ...filters.filter(
          (filter) =>
            filter.key !== "assigned_role" && filter.key !== "content_owner",
        ),
        myDocumentFilter,
      ];
    }
            ...
            <TableBody>
              {table.getRowModel().rows.length > 0 ? (
                table.getRowModel().rows.map((row) => {
                  ...
                  const isLockedByOther = !isUnlocked && !isLockedByMe;

                  return (
                    <TableRow
                      key={row.id}
                      className={isLockedByOther ? "bg-[#e6e8e8]" : undefined}
                    >
                      <FavoriteStar
                        doc={doc}
                        onToggleOn={(item) => toggleFavorite(item as documentContent, false)}
                        onToggleOff={(item) => toggleFavorite(item as documentContent, true)}
                      />
                          ...
                          <div className="flex items-center gap-2 justify-end">
                            ...
                            {canEdit && isLockedByMe && (
                              <Button
                                ...
                                onClick={() => lockDocument(doc, false)}
                              >
                                <LockOpen />
                              </Button>
                            )}
